    ...
    let
        createOrGoBackToEditedRoutine =
            if model.hasUnsavedChanges then
                returnToEditedRoutine model.routineId
...
returnToEditedRoutine : Maybe RoutineId -> Element msg
returnToEditedRoutine maybeRoutineId =
    E.el [ E.paddingXY 0 5 ]
        (E.link Common.buttonAttrs
            { url = Router.href (Router.RoutineEditor maybeRoutineId)
